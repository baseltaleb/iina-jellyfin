    ...
    <div className={'poster-card sz-' + size} onClick={onClick}>
      <div className="poster-art">
        ...
        {typeof progress === 'number' && progress > 0 && progress < 1 && (
          <div className="poster-progress">
            <div className="poster-progress-fill" style={{ width: progress * 100 + '%' }}></div>
          </div>
        )}
        {watched === 1 && (
          <div className="poster-watched">
      ...
      <div className="poster-info">
        ...
        <div className="poster-meta">
          {subtitle || [year, genres && genres[0]].filter(Boolean).join(' · ')}
        </div>
      </div>
    </div>
  );
...
function EpisodeCard({ item, onClick, session }) {
  ...
  var imgUrl =
    session && item.episodeId && !imgFailed
      ? INAFIN.getImageUrl(session, item.episodeId, 400)
      : null;
        ...
        <div className="topbar-nav">
          {['home', 'movies', 'shows', 'recent'].map(function (p) {
            ...
            return (
              <button
                key={p}
                className={'topbar-nav-item' + (page === p ? ' on' : '')}
                onClick={function () {
                  onNav(p);
                }}
              >
      ...
      <nav>
        {items.map(function (it) {
          return (
            <button
              key={it.id}
              className={'sidebar-item' + (page === it.id ? ' on' : '')}
              onClick={function () {
                onNav(it.id);
              }}
            >
...
function SearchPalette({ open, onClose, onSelect, session }) {
  ...
  var allResults = results.movies
    .map(function (m) {
      return { type: 'movie', data: m };
    })
    .concat(
      results.shows.map(function (s) {
        return { type: 'show', data: s };
      })
    );
  ...
  function handleKey(e) {
    ...
    if (e.key === 'ArrowDown') {
      ...
      setFocusIdx(function (i) {
        return Math.min(i + 1, allResults.length - 1);
      });
    }
    if (e.key === 'ArrowUp') {
      e.preventDefault();
    ...
    if (e.key === 'Enter' && allResults[focusIdx]) {
      onSelect(allResults[focusIdx]);
      onClose();
    }
  }
    ...
    >
      <div className="search-box" onKeyDown={handleKey}>
        ...
        <div className="search-results">
          ...
          {!searching && query && allResults.length === 0 && (
            <div className="search-empty">No results for "{query}"</div>
          )}
          {!searching && results.movies.length > 0 && (
            <div className="search-group-label">MOVIES</div>
          )}
          {!searching &&
            results.movies.map(function (m, i) {
              var idx = i;
              return (
                <button
                  key={m.id}
                  className={'search-item' + (focusIdx === idx ? ' on' : '')}
                  onClick={function () {
                    onSelect({ type: 'movie', data: m });
                    onClose();
                  }}
                ...
                >
                  <div
                    ...
                    style={{ background: INAFIN.posterGradient(m.title) }}
                  >
                    {session && (
                      <img
                        src={INAFIN.getImageUrl(session, m.id, 64)}
                        alt=""
                        onError={function (e) {
                  ...
                  <div className="search-item-info">
                    ...
                    <div className="search-item-meta">
                      {[m.year, m.genres && m.genres[0]].filter(Boolean).join(' · ')}
                    </div>
                  </div>
                </button>
              );
          ...
          {!searching && results.shows.length > 0 && (
            <div className="search-group-label">SERIES</div>
          )}
          {!searching &&
            results.shows.map(function (s, j) {
              ...
              return (
                ...
                >
                  <div
                    className="search-item-poster"
                    style={{ background: INAFIN.posterGradient(s.title) }}
                  >
                    {session && (
                      <img
                        src={INAFIN.getImageUrl(session, s.id, 64)}
                        alt=""
                        onError={function (e) {
                          e.target.style.display = 'none';
                        }}
...
function ToastProvider({ children }) {
  ...
  function addToast(msg) {
    ...
    setToasts(function (t) {
      return t.concat({ id: id, msg: msg });
    });
    setTimeout(function () {
      setToasts(function (t) {
        return t.map(function (x) {
          return x.id === id ? Object.assign({}, x, { leaving: true }) : x;
        });
      });
      setTimeout(function () {
        setToasts(function (t) {
